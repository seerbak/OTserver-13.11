...
function Creature:onDrainHealth(attacker, typePrimary, damagePrimary,
	...
	if self:isPlayer() then
		if damagePrimary > 0 and self:hasResilience(typePrimary) then
			damagePrimary = reduceEightPercent(damagePrimary)
		end
		if damageSecondary > 0 and self:hasResilience(typeSecondary) then
			damageSecondary = reduceEightPercent(damageSecondary)
		end
	end
	
	if attacker then
		if attacker:isPlayer() then
			if damagePrimary > 0 and attacker:hasAmplification(typePrimary) then
				damagePrimary = increaseEightPercent(damagePrimary)
			end
			if damageSecondary > 0 and attacker:hasAmplification(typeSecondary) then
				damageSecondary = increaseEightPercent(damageSecondary)
			end
		end
	end
